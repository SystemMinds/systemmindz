...
const getEmailConfig = async () => {
    // Direct env-var fallback — works without a superadmin backend
    if (process.env.ZEPTO_API_KEY && process.env.ZEPTO_FROM_EMAIL) {
        return {
            ...
            fromName:  process.env.ZEPTO_FROM_NAME  || 'SystemMindz',
            apiUrl:    process.env.ZEPTO_API_URL     || 'https://api.zeptomail.in/v1.1/email',
        };
    }
...
const sendEmail = async (to, subject, htmlBody, cc, attachments, options = {}) => {
    ...
    const fromName = (emailConfig.fromName || 'SystemMindz').trim();
...
const sendCandidateInvitationEmail = async (to, candidateName, adminName, tempPassword, loginUrl) => {
    const subject = `Invitation to join SystemMindz from ${adminName}`;
    const htmlBody = `
        <div style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; border: 1px solid #eee; padding: 20px; border-radius: 10px;">
            <h2 style="color: #f97316;">Hello ${candidateName}!</h2>
            <p><strong>${adminName}</strong> has invited you to join the <strong>SystemMindz</strong> platform.</p>
            <div style="background-color: #f9f9f9; padding: 15px; border-radius: 5px; margin: 20px 0;">
                <p style="margin: 0;"><strong>Your Login Credentials:</strong></p>
                <p style="margin: 10px 0 0 0;">Email: <span style="color: #f97316;">${to}</span></p>
                <p style="margin: 5px 0 0 0;">Password: <span style="color: #f97316;">${tempPassword}</span></p>
            </div>
            <p>Login here: <a href="${loginUrl}" style="color: #f97316; font-weight: bold;">${loginUrl}</a></p>
            <p style="margin-top: 30px; font-size: 0.9em; color: #777;">
                Best regards,<br>Team SystemMindz
            </p>
        </div>
    `;
...
/**
 * Send notification email to candidate when removed from a position.
 */
const sendCandidateRemovalEmail = async (to, candidateName, positionTitle) => {
    ...
    const htmlBody = `
        <div style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; border: 1px solid #eee; padding: 20px; border-radius: 10px;">
            ...
            <p>We are writing to inform you that your profile has been removed from this position's active list at this time.</p>
            <p>We wish you the very best in your professional journey.</p>
            <p style="margin-top: 30px; font-size: 0.9em; color: #777;">
                Best regards,<br>Recruitment Team, SystemMindz
            </p>
        </div>
    `;
    return await sendEmail(to, subject, htmlBody);
